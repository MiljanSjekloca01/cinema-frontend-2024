<script setup lang="ts">
  ...
  import { useRouter } from 'vue-router';
    ...
    const today = new Date().toISOString().split('T')[0]; // Danas
    ...
    async function createProjection(projection: any) {
        if (projection.startsAt && projection.endsAt && projection.projectionDate) {
            await ProjectionService.createProjection(projection).then(rsp => {
                if(rsp.status === 500){
                    alert("Projection is already happening in Hall at this time")
                    return;
                }else{
...
<template>
    <div class="container">
      <h1 class="h3 text-center">Create Projection</h1>
      <form>
        <div class="mb-3">
      ...
      <RouterLink class="btn btn-md btn-secondary" to="/admin-panel/projection">
        ...
        Return to Projections List
      </RouterLink>&nbsp;
      <button type="button" class="btn btn-md btn-success"
        @click="createProjection(projection)" :disabled="validationCheck(projection)">
